...
public:
    explicit Camera(const window::Resolution& res) :
        position_(-6.0F, 0.0F, 2.0F), front_(1, 0, 0), up_(0, 0, 1), right_(0, -1, 0), world_up_(0, 0, 1),
        projection_matrix_(math::perspective_fov(65.F, res.aspect_ratio, 0.1F, 100.0F)), move_speed_(5.0F) {
        ...
        add_handler_down(Action::CameraLook,
                         {.callback = [this](const Action act, const type t) { setLooking(act, t); }});
        add_handler_up(Action::CameraLook,
                       {.callback = [this](const Action act, const type t) { setLooking(act, t); }});
...
private:
    void updatePos(const math::scalar dt) {
        ...
        if (is_moving_.at(Bckwd))
            position_ += speed_factor * -front_;
        if (is_moving_.at(Up))
            position_ += speed_factor * world_up_;
    ...
    void updateFront() {
        ...
        const math::xvec3 new_front = {r * math::cos(math::radians(yaw_)), r * math::sin(math::radians(yaw_)),
