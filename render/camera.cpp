...
Camera::Camera(const window::Resolution& res) :
    projection_matrix_(math::perspective_fov(65.f, res.aspect_ratio, 0.1f, 100.0f)), position_(-6.0f, 0.0f, 2.0f),
...
void Camera::updatePos(math::scalar dt) {
    ...
    if (is_moving_[Dir::Fwd])
        position_ += speed_factor * front_;
    if (is_moving_[Dir::Bckwd])
        position_ += speed_factor * -front_;
    if (is_moving_[Dir::Up])
        position_ += speed_factor * world_up_;
    if (is_moving_[Dir::Down])
        position_ += speed_factor * -world_up_;
    if (is_moving_[Dir::Right])
        position_ += speed_factor * right_;
    if (is_moving_[Dir::Left])
        position_ += speed_factor * -right_;
}
...
void Camera::updateFront() {
    ...
    new_front = {r * math::cos(math::radians(yaw_)), r * math::sin(math::radians(yaw_)),
