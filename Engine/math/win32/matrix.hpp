...
public:
    INLINE mat3() {}
    INLINE mat3(vec3 x, vec3 y, vec3 z) : mat3_({x, y, z}) {}
    ...
    [[nodiscard]] INLINE vec3 GetX() const { return mat3_[0]; }
    [[nodiscard]] INLINE vec3 GetY() const { return mat3_[1]; }
    [[nodiscard]] INLINE vec3 GetZ() const { return mat3_[2]; }
    INLINE void SetX(vec3 x) { mat3_[0] = x; }
    INLINE void SetY(vec3 y) { mat3_[1] = y; }
    INLINE void SetZ(vec3 z) { mat3_[2] = z; }

    INLINE mat3 operator*(scalar scalar) const { return mat3(scalar * mat3_[0], scalar * mat3_[1], scalar * mat3_[2]); }
    INLINE vec3 operator*(vec3 vec) const { return vec3(XMVector3TransformNormal(vec, *this)); }
    ...
    INLINE mat3 MakeScale(vec3 scale) { return mat3(XMMatrixScalingFromVector(scale)); }
private:
     std::array<vec3, 3> mat3_;
};

__declspec(align(16)) class mat4 {
public:
    INLINE mat4() {}
    INLINE mat4(vec3 x, vec3 y, vec3 z, vec3 w) : mat4_({
                 XMVectorSetW(x, 0.0f),
                 XMVectorSetW(y, 0.0f),
                 XMVectorSetW(z, 0.0f),
                 XMVectorSetW(w, 1.0f) }) {}
    INLINE mat4(const float *data) { mat4_ = XMLoadFloat4x4((XMFLOAT4X4 *)data); }
    INLINE mat4(vec4 x, vec4 y, vec4 z, vec4 w) : mat4_(x,y,z,w) {}
    ...
    INLINE mat4(const mat3 &xyz, vec3 w) : mat4_(
    ...
    [[nodiscard]] INLINE vec4 GetX() const { return mat4_.r[0]; }
    [[nodiscard]] INLINE vec4 GetY() const { return mat4_.r[1]; }
    [[nodiscard]] INLINE vec4 GetZ() const { return mat4_.r[2]; }
    [[nodiscard]] INLINE vec4 GetW() const { return mat4_.r[3]; }
    INLINE void SetX(vec4 x) { mat4_.r[0] = x; }
    INLINE void SetY(vec4 y) { mat4_.r[1] = y; }
    INLINE void SetZ(vec4 z) { mat4_.r[2] = z; }
    INLINE void SetW(vec4 w) { mat4_.r[3] = w; }

    INLINE operator XMMATRIX() const { return mat4_; }
    INLINE vec4 operator*(vec3 vec) const { return XMVector3Transform(vec, mat4_); }
    INLINE vec4 operator*(vec4 vec) const { return XMVector4Transform(vec, mat4_); }
    ...
    INLINE mat4 MakeScale(vec3 scale) { return XMMatrixScalingFromVector(scale); }
private:
    XMMATRIX mat4_;
};
