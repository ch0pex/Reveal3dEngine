...
T sqrt(T s) {
    return XMVectorSqrt(s);
}
template<is_vector T>
T recip(T s) {
    return T(XMVectorReciprocal(s));
}
template<is_vector T>
T recip_sqrt(T s) {
    return T(XMVectorReciprocalSqrt(s));
}
template<is_vector T>
T floor(T s) {
    return T(XMVectorFloor(s));
}
template<is_vector T>
T ceiling(T s) {
    return T(XMVectorCeiling(s));
}
template<is_vector T>
T round(T s) {
    return T(XMVectorRound(s));
}
template<is_vector T>
T abs(T s) {
    return T(XMVectorAbs(s));
}
template<is_vector T>
T exp(T s) {
    return T(XMVectorExp(s));
}
template<is_vector T>
T pow(T b, T e) {
    return T(XMVectorPow(b, e));
}
template<is_vector T>
T log(T s) {
    return T(XMVectorLog(s));
}
template<is_vector T>
T sin(T s) {
    return T(XMVectorSin(s));
}
template<is_vector T>
T cos(T s) {
    return T(XMVectorCos(s));
}
template<is_vector T>
T tan(T s) {
    return T(XMVectorTan(s));
}
template<is_vector T>
T a_sin(T s) {
    return T(XMVectorASin(s));
}
template<is_vector T>
T a_cos(T s) {
    return T(XMVectorACos(s));
}
template<is_vector T>
T a_tan(T s) {
    return T(XMVectorATan(s));
}
template<is_vector T>
T a_tan2(T y, T x) {
    return T(XMVectorATan2(y, x));
}
template<is_vector T>
T lerp(T a, T b, T t) {
    return T(XMVectorLerpV(a, b, t));
}
template<is_vector T>
T lerp(T a, T b, f32 t) {
    return T(XMVectorLerp(a, b, t));
}
template<is_vector T>
T max_(T a, T b) {
    return T(XMVectorMax(a, b));
}
template<is_vector T>
T min_(T a, T b) {
    return T(XMVectorMin(a, b));
}
template<is_vector T>
T clamp(T v, T a, T b) {
    return min_(max_(v, a), b);
}
template<is_vector T>
BoolVector operator<(T lhs, T rhs) {
...
template<is_vector T>
T select(T lhs, T rhs, BoolVector mask) {
    return T(XMVectorSelect(lhs, rhs, mask));
}
...
inline mat4 look_at(xvec3 position, xvec3 focus_point, xvec3 up_dir) {
    return XMMatrixLookAtLH(position, focus_point, up_dir);
}
inline mat4 perspective_fov(f32 fov, f32 aspect_ratio, f32 near_plane, f32 far_plane) {
