...
void Gpass::setRenderTargets(Commands& command_mng, FrameResource& frame_resource) {
    ...
    command_list->ClearRenderTargetView(frame_resource.back_buffer_handle.cpu,
                                        math::utils::to_array(Config::scene.clearColor).data(), 0, nullptr);
    command_list->ClearDepthStencilView(frame_resource.depth_buffer_handle.cpu,
                                        D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 0, 0, nullptr);
    command_list->OMSetRenderTargets(1, &frame_resource.back_buffer_handle.cpu, TRUE,
                                     &frame_resource.depth_buffer_handle.cpu);
}
    ...
    for (auto& render_element: render_elements_) {
        ...
        for (auto& submesh: geometry.subMeshes()) {
            ...
            command_list->SetGraphicsRootConstantBufferView(
                    0, frame_resource.constant_buffer.gpuPos(id::index(transform.id()))); // 2 DWORDS
            command_list->SetGraphicsRootConstantBufferView(
                    1, frame_resource.mat_buffer.gpuPos(id::index(geometry.id()))); // 2 DWORDS
...
void Gpass::addRenderElement(core::Entity entity, Commands& cmd_mng, ID3D12Device* device) {
    ...
    BufferInitInfo vertex_buffer_info = {.device = device,
                                         .cmd_list = cmd_mng.list(),
                                         .data = geometry.vertices().data(),
                                         .count = geometry.vertexCount()};

    BufferInitInfo index_buffer_info = {.device = device,
                                        .cmd_list = cmd_mng.list(),
...
void Gpass::buildPsos(ID3D12Device* device) {
    ...
    // TODO Config file for assets path
    hr = D3DCompileFromFile(relative(L"../../Assets/shaders/hlsl/FlatShader.hlsl").c_str(), nullptr, nullptr, "VS",
                            "vs_5_0", compile_flags, 0, &vertex_shader, &errors);
    if (errors != nullptr)
        logger(LogDebug) << (char*)errors->GetBufferPointer();
    hr >> utl::DxCheck;
    hr = D3DCompileFromFile(relative(L"../../Assets/shaders/hlsl/FlatShader.hlsl").c_str(), nullptr, nullptr, "PS",
                            "ps_5_0", compile_flags, 0, &pixel_shader, &errors);
    if (errors != nullptr)
        logger(LogDebug) << (char*)errors->GetBufferPointer();
    hr >> utl::DxCheck;
