...
void Dx12::initDsBuffer() {
    ...
    const D3D12_CLEAR_VALUE opt_clear = {.Format = DXGI_FORMAT_D24_UNORM_S8_UINT,
                                         .DepthStencil = {
                                                 .Depth = 1.0f,
                                                 .Stencil = 0,
                                         }};
    ...
    device_->CreateCommittedResource(&heap_prop, D3D12_HEAP_FLAG_NONE, &depth_stencil_desc,
                                     D3D12_RESOURCE_STATE_DEPTH_WRITE, &opt_clear,
                                     IID_PPV_ARGS(depth_stencil_buffer_.GetAddressOf())) >>
            utl::DxCheck;

    const D3D12_DEPTH_STENCIL_VIEW_DESC dsv_desc = {.Format = DXGI_FORMAT_D24_UNORM_S8_UINT,
